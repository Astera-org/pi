...
<a id="pi-replacetranscript"></a>

`pi.replaceTranscript(messages, options?)` writes a durable boundary: context construction stops there and uses `messages` instead, on every future request, until a later boundary supersedes it. Unlike the [`context`](#context_with_system) event, which only reshapes a single request, this is recorded in the session and survives reload and branch navigation. Earlier entries stay on disk for history, forking, and tree navigation, but never reach the model again.

By default (`deliverAs: "steer"`) the replacement takes effect at the end of the current turn, before the next model call; `"followUp"` waits for the run to fully settle, and `"nextTurn"` waits for the next user prompt. A replacement is never applied between a tool call and its result. See [skill-state.ts](../examples/extensions/skill-state.ts) for a complete example that holds context at O(1) across an arbitrarily long run.
