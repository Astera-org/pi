...
export function prepareCompaction(
	...
	// A "transcript" boundary (replaceTranscript()) is handled for free here too: buildContextEntries()
	// already slices projectedEntries to start at the latest boundary of either type, so when the
	// boundary is a transcript entry, prevCompactionIndex stays -1 and boundaryStart is 0 - the transcript
	// entry itself and everything after it are eligible for summarization, with no carried-over summary.
